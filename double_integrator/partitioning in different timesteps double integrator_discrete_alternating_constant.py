...
# ord2 = define which dimension to be partioned firstly in the second prediction step
...
# in this approach in each timestep another dimension is partitioned in an alternating way
...
# each cutvector contains the number of cuts in the respective dimensions for each step     
    ...
    cut2 = np.zeros((nx,1))
    cut2[0] = x1 # cuts in the first dimension  (0)
    cut2[1] = x0 # cuts in the second dimension (1)
...
# objective and equality constraints should be the same since ns (number of subregions) remains same, but the alignment changes in every timestep            
    for i in range(N):
        ...
        for s in range(ns):
            ...
            # constraints to keep the u-deviations small
            #if i > 0:
                #1)
                #g.append(opt_x['u',i,s]-opt_x['u',i-1,s])
                #lb_g.append(-np.ones((nu,1))*epsilon)
                #ub_g.append(np.ones((nu,1))*epsilon)
                #2)
                #g.append(norm_2(opt_x['u',i,s]-sum(opt_x['u',i-1,:])/len(opt_x['u',i-1,:])))
                #lb_g.append(0)
                #ub_g.append(epsilon)
# terminal cost                
...
# cutting1 (first cut the 0th dimension) 
...
# cutting2 (first cut the 1 dimension) 
...
# computation of 2-step RCIS
    ...
    for i in range(N_RCIS):
        ...
        if i%2 == 0:
            ax[i].set_title("partitioning step 1",fontweight = 'bold',fontsize = 24)
        elif i%2 == 1:
            ax[i].set_title("partitioning step 2",fontweight = 'bold',fontsize = 24)
    ...
    fig.align_labels()
    suptitle = fig.suptitle("{0}-step RCIS (alternating partitioning[ns={1}])".format(N_RCIS,ns),fontweight = 'bold', fontsize=28,y = 1.1)
...
# depending on which dimension was cut lastly we should adjust in the corresponding set of the RCIS so that x(N)ExRCIS holds
...
# set the problem and initialize the optimizer for the first cutting case (partitioning of 0th dimension first)
...
# set the problem and initialize the optimizer for the second cutting case (partitioning of 1 dimension first)
        ...
        if j>0:
            if j % 2 == 0: #cut in the 0th dimension in the first (odd) prediction step (1)
                if odd_count1 > even_count1: # set the corresponding terminal constraint according to the N prediction steps
                    ...
                    mpc_res = mpc_mon_solver_cut1(p=vertcat(x_0,pmax,pmin,u_k,x_sp,np.array(res['x_max',1,-1]),np.array(res['x_min',1, 0])), x0=opt_x_k, lbg=lb_g1, ubg=ub_g1, lbx = lb_opt_x, ubx = ub_opt_x)
            elif j % 2 == 1:  #cut in the 1 dimension in the first (odd) prediction step (1)
                if odd_count2 > even_count2:
                    mpc_res = mpc_mon_solver_cut2(p=vertcat(x_0,pmax,pmin,u_k,x_sp,np.array(res['x_max',1,-1]),np.array(res['x_min',1, 0])), x0=opt_x_k, lbg=lb_g2, ubg=ub_g2, lbx = lb_opt_x, ubx = ub_opt_x)
            ...
            if odd_count1 > even_count1: # set the corresponding terminal constraint according to the N prediction steps
                ...
                mpc_res = mpc_mon_solver_cut1(p=vertcat(x_0,pmax,pmin, uinitial,x_sp,np.array(res['x_max',1,-1]),np.array(res['x_min',1, 0])), x0=opt_x_k, lbg=lb_g1, ubg=ub_g1, lbx = lb_opt_x, ubx = ub_opt_x)         
...
#        fig.align_labels()
    ...
    datadict = {'CL': [mpc_mon_cut_res,CL_time], 'OL': [opt,sol], 'ns': [subreg],'RCIS':[RCIS_total,RCIS_step1,RCIS_step2]}
    
    return datadict

# use the do-mpc sampler to run multiple experiments of the alternaing constant approach in order to be able to create contour plots and obtain more knowledge about how the system behaves with the partitioning approach

# sp = do_mpc.sampling.SamplingPlanner()
# sp.set_param(overwrite = False)
# sp.data_dir = './alternating_constant/'

# sp.set_sampling_var('seed')
# sp.set_sampling_var('cut0')
# sp.set_sampling_var('cut1')
# sp.set_sampling_var('ord1')
# sp.set_sampling_var('ord2')

# #create a sampling plan with n_samples = 400 samples (4 levels for ordering x 10 levels for 10 partitions in dim0 x 10 levels for 10 partitions in dim1)
...
#     for x00 in x0:
#         for x10 in x1:
#             plan = sp.add_sampling_case(seed = case ,cut0 = x00,cut1 = x10,ord1 = ord[0],ord2 = ord[1])


# sp.export('sampling_plan_alternating_constant')

# #due to the varying flexibility for the different approaches a different sampling plan for the different partitioning approaches must be used 

# sampler = do_mpc.sampling.Sampler(plan)
# sampler.set_param(overwrite = False)

# def sample_function(seed,cut0,cut1,ord1,ord2):
#     return closed_loop_comp(seed,cut0,cut1,ord1,ord2)

# sampler.set_sample_function(sample_function)

# sampler.data_dir = './alternating_constant/'
# sampler.set_param(sample_name = 'alternating constant')
# sampler.sample_data()
...
datadict_CL_run_constant = closed_loop_comp(0,15,0,[0,1],[0,1])
    
np.save('datadict_CL_run_constant_16_one.npy',datadict_CL_run_constant)

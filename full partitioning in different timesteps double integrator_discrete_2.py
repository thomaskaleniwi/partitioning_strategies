...
# computation of 2-step RCIS
    ...
    for i in range(N_RCIS):
        ...
        ax[i].grid(False)
        ax[i].set_title("{0}-step".format(i+1),fontweight = 'bold',fontsize = 24,pad = 12)
...
# long term short: for the full-partitioning case two solvers will be used but they are the same since the problem is defined equally and the switching in the even/odd simulation steps will not have an effect
    ...
    prob = {'f':J,'x':vertcat(opt_x),'g':g, 'p':vertcat(x_init,p_plus,p_minus,u_bef,x_ref, x_RCIS_plus, x_RCIS_minus)}
...
# run the closed-loop
    ...
    # solve optimization problem 
        ...
        if j>0:           
            mpc_res = mpc_mon_solver_cut(p=vertcat(x_0,pmax,pmin,u_k,x_sp,np.array(res['x_max',0,-1]),np.array(res['x_min',0, 0])), x0=opt_x_k, lbg=lb_g, ubg=ub_g, lbx = lb_opt_x, ubx = ub_opt_x)
        else: # j = 0 
            print('Run a first iteration to generate good Warmstart Values')            
            mpc_res = mpc_mon_solver_cut(p=vertcat(x_0,pmax,pmin, uinitial,x_sp,np.array(res['x_max',0,-1]),np.array(res['x_min',0, 0])), x0=opt_x_k, lbg=lb_g, ubg=ub_g, lbx = lb_opt_x, ubx = ub_opt_x)
...
datadict_CL_run_full = closed_loop_comp(0)
    
np.save('datadict_CL_run_full.npy',datadict_CL_run_full)

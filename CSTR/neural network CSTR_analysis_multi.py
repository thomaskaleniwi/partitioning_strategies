...
# normalize training-data-set over **samples** und **timesteps**
...
# Flatten into a long list of single predictions
n_seq, H, _ = X_test_norm.shape
Xf = X_test_norm.reshape(-1, 17)            # (n_seq*H, 17)
Yf = Y_test.reshape(-1,        4)           # (n_seq*H,  4)
...
y_std_np  = y_std.squeeze().cpu().numpy()  # shape (1,4)
y_mean_np = y_mean.squeeze().cpu().numpy() # shape (1,4)
y_pred_flat = y_norm_pred * y_std_np + y_mean_np  # shape (batch,4)

y_true_flat = Yf.numpy()
...
for i, ax in enumerate(axes.flat):
    ax.scatter(y_true_flat[:, i], y_pred_flat[:, i], alpha=0.5)
    mn = min(y_true_flat[:,i].min(), y_pred_flat[:,i].min())
    mx = max(y_true_flat[:,i].max(), y_pred_flat[:,i].max())
    ax.plot([mn,mx],[mn,mx],'k--')
    ax.set_xlabel(f"True {state_names[i]}")
    ax.set_ylabel(f"Pred {state_names[i]}")
    ax.grid(False)
fig.tight_layout()
plt.savefig("pairplot_multistep_NN_CSTR.pdf", format="pdf")
plt.savefig("pairplot_nultistep_NN_CSTR.svg", format="svg")
plt.show()
...
for i, ax in enumerate(axes2.flat):
    ...
    ax.grid(False)
fig2.tight_layout()
plt.savefig("pairplot_mutlistep_NN_CSTR.pdf", format="pdf")
plt.savefig("pairplot_mutlistep_NN_CSTR.svg", format="svg")
plt.show()

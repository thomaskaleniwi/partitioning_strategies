...
class CSTRDynamicsNN(nn.Module):
    def __init__(self, dim_x=4, dim_u=2, dim_p=11, dropout_p = 0.0005):
        ...
        # self.mon2d = nn.Linear(hidden, self.dim_x, bias=False)
...
W2c     = to_numpy(model_NN.mon2c.weight)

# extract positive and negative weights from W2f to build decompositionfunction

mask2c_pos = (W2c > 0)
mask2c_neg = (W2c < 0)
W2c_pos = W2c*mask2c_pos
W2c_neg = W2c*mask2c_neg
...
hu = vertcat(h1b, u)
h2a = tanh(W2a @ hu + b2a)
h2b = tanh(W2b @ h2a + b2b)

z = h2b

z_next = z # z-prediction for decomposition-fct
x_next = W2c @ z # state prediction
...
rhs_teta_min.append(uF*(teta0-teta_min)+(kw_max*Ar*(teta_K_min-teta_min))/(rho_min*Cp_min*Vr)-(k1_max*exp(-EA_1R_0/(teta_min+273.15))*cA_max*deltaH_AB_max/(rho_min*Cp_min))-(k2_min*exp(-EA_2R_0/(teta_min+273.15))*cB_min*deltaH_BC_max/(rho_max*Cp_max))-(k3_min*exp(-EA_3R_0/(teta_min+273.15))*cA_min*cA_min*deltaH_AD_max/(rho_max*Cp_max)))
...
rhs_cA_max.append(uF*(cA0_max-cA_max)-k1_min*exp(-EA_1R_0/(273.15+teta_max))*cA_max-k3_min*exp(-EA_3R_0/(273.15+teta_max))*cA_max*cA_max)
rhs_cB_max.append(-uF*cB_max+k1_max*exp(-EA_1R_0/(273.15+teta_max))*cA_max-k2_min*exp(-EA_2R_0/(273.15+teta_max))*cB_max)
rhs_teta_max.append(uF*(teta0-teta_max)+(kw_min*Ar*(teta_K_max-teta_max))/(rho_max*Cp_max*Vr)-(k1_min*exp(-EA_1R_0/(teta_max+273.15))*cA_min*deltaH_AB_min/(rho_max*Cp_max))-(k2_max*exp(-EA_2R_0/(teta_max+273.15))*cB_max*deltaH_BC_min/(rho_min*Cp_min))-(k3_max*exp(-EA_3R_0/(teta_max+273.15))*cA_max*cA_max*deltaH_AD_min/(rho_min*Cp_min)))
...
# rhs_cA_max.append(uF*(cA0_max-cA_max)-k1_min*exp(-EA_1R_0/(273.15+teta_min))*cA_max-k3_min*exp(-EA_3R_0/(273.15+teta_min))*cA_max**2)
# rhs_cB_max.append(-uF*cB_max+k1_max*exp(-EA_1R_0/(273.15+teta_max))*cA_max-k2_min*exp(-EA_2R_0/(273.15+teta_max))*cB_max)
...
sim_p = []
for i in range(10): #create nominal trajectories with both pmax and pmin but for nominal inputs
    if i == 0:
        simulator.set_tvp_fun(p_max)
        simulator.setup()
    elif i == 1: 
        ...
        simulator.set_tvp_fun(lambda t:p_fun_var(i,8,False))
        simulator.setup()

    x_0 = np.array([[0],[0],[100],[100]])/scaling_x
    simulator.reset_history()
    simulator.x0 = x_0*ub_x
    N_sim = 20
    ...
    sim_p.append(copy.copy(simulator.data))
...
ax_dict = fig.subplot_mosaic(
    [
        ["cA", "cB"],
        ["teta","teta_K"]
    ],empty_sentinel="X", gridspec_kw = {"wspace" : 0.1, "hspace" : 0.3}
)
...
for q in range(N_sim):
    ...
    if q == 0:
        ...
        x_prev_max = normalize(x0, x_mean, x_std)
        x_prev_min = normalize(x0, x_mean, x_std)
        u_prev = normalize(u0, u_mean, u_std)
        p_prev = normalize(p_max, p_mean, p_std)
        ...
        x_norm_max = d(x_prev_max,x_prev_max, u_prev, p_prev_max,p_prev_min)
        x_norm_min = d(x_prev_min,x_prev_min, u_prev, p_prev_min,p_prev_max)
        ...
        u_now = u_sim[q-1]   # aus Closed-Loop-Datensatz
...
# for comparison with more conservative decomposition-fct. from interval arithmetics simulate these trajectories with cvodes integrator

# p_minus = np.concatenate((np.array([(1-cA0_var)*cA0s]),np.array([(1-k1_var)*k1_0]),np.array([(1+k2_var)*k2_0]),np.array([(1+k3_var)*k3_0]),np.array([(1+deltaH_AB_var)*deltaH_AB_0]),np.array([(1-deltaH_BC_var)*deltaH_BC_0]),np.array([(1-deltaH_AD_var)*deltaH_AD_0]),np.array([(1+rho_var)*rho_0]),np.array([(1+Cp_var)*Cp_0]),np.array([(1+Cpk_var)*Cpk_0]),np.array([(1-kw_var)*kw_0])),axis=0)
# p_plus = np.concatenate((np.array([(1+cA0_var)*cA0s]),np.array([(1+k1_var)*k1_0]),np.array([(1-k2_var)*k2_0]),np.array([(1-k3_var)*k3_0]),np.array([(1-deltaH_AB_var)*deltaH_AB_0]),np.array([(1+deltaH_BC_var)*deltaH_BC_0]),np.array([(1+deltaH_AD_var)*deltaH_AD_0]),np.array([(1-rho_var)*rho_0]),np.array([(1-Cp_var)*Cp_0]),np.array([(1-Cpk_var)*Cpk_0]),np.array([(1+kw_var)*kw_0])),axis=0)
...
x_min_sym = SX.sym('x_min', 4)
x_max_sym = SX.sym('x_max', 4)
u_sym     = SX.sym('u',    2)
pmin_sym  = SX.sym('pmin', 11)
pmax_sym  = SX.sym('pmax', 11)

dx_min = dmin(x_min_sym, x_max_sym, u_sym, pmin_sym, pmax_sym)
dx_max = dmax(x_min_sym, x_max_sym, u_sym, pmin_sym, pmax_sym)
rhs    = vertcat(dx_min, dx_max)

# 3) Kombiniere die Zustände zu einem einzigen Vektor z = [x_min; x_max]
z = vertcat(x_min_sym, x_max_sym)
# und die Eingänge/Parameter zu p = [u; pmin; pmax]
p = vertcat(u_sym, pmin_sym, pmax_sym)

# 4) Erstelle den CVODES-Integrator mit Zeitschritt dt
dt = 0.1
opts = {
    'tf': dt,           # Integrationszeit
    'abstol': 1e-8,     # absolute Toleranz
    'reltol': 1e-8,     # relative Toleranz
    # optional: weitere CVODES-Optionen wie max_num_steps
}
I = integrator('I', 'cvodes', {'x': z, 'p': p, 'ode': rhs}, opts)

# 5) Simulation über eine Trajektorie
def simulate_with_cvodes(u_traj, x_min0, x_max0):
    N = u_traj.shape[0]-1
    z0 = np.concatenate([x_min0, x_max0])
    traj = np.zeros((N+1, 8))
    traj[0] = z0
    for k in range(N):
        pv = np.concatenate([u_traj[k], p_minus, p_plus])
        sol = I(x0=z0, p=pv)
        z0  = sol['xf'].full().flatten()
        traj[k+1] = z0
    x_min_traj = traj[:, :4]
    x_max_traj = traj[:, 4:]
    return x_min_traj, x_max_traj

x_min_d, x_max_d = simulate_with_cvodes(u_sim, x_sim[0], x_sim[0])

# states 
ax_dict["cA"].plot(t_sim,dd['CL'][0]['_x','cA'],label = "cA_CL",alpha = 1 ,color ='tab:blue')
...
ax_dict["cA"].plot(t_sim,sim_p[9]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[8]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[7]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[6]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[5]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[4]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[3]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[2]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[1]['_x','cA'],alpha = 1 ,color = 'tab:blue')
ax_dict["cA"].plot(t_sim,sim_p[0]['_x','cA'],alpha = 1 ,color = 'tab:blue')
# ax_dict["cA"].plot(t_sim,cA_NN_max,label = "cA_NN",color='tab:orange')
# ax_dict["cA"].plot(t_sim,cA_NN_min,color='tab:orange')
ax_dict["cA"].plot(t_sim,x_max_d[:,0],label = "cA_relax",color='tab:orange')
ax_dict["cA"].plot(t_sim,x_min_d[:,0],color='tab:orange')
...
ax_dict["cB"].plot(t_sim,dd['CL'][0]['_x','cB'],label = "cB_CL",alpha = 1 ,color ='tab:blue')
...
ax_dict["cB"].plot(t_sim,sim_p[9]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[8]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[7]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[6]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[5]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[4]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[3]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[2]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[1]['_x','cB'],alpha = 1 ,color = 'tab:blue')
ax_dict["cB"].plot(t_sim,sim_p[0]['_x','cB'],alpha = 1 ,color = 'tab:blue')
# ax_dict["cB"].plot(t_sim,cB_NN_max,label = "cB_NN",color='tab:orange')
# ax_dict["cB"].plot(t_sim,cB_NN_min,color='tab:orange')
ax_dict["cB"].plot(t_sim,x_max_d[:,1],label = "cB_relax",color='tab:orange')
ax_dict["cB"].plot(t_sim,x_min_d[:,1],color='tab:orange')
...
ax_dict["teta"].plot(t_sim,dd['CL'][0]['_x','teta'],label = "teta_CL",alpha = 1 ,color ='tab:blue')
...
ax_dict["teta"].plot(t_sim,sim_p[9]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[8]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[7]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[6]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[5]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[4]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[3]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[2]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[1]['_x','teta'],alpha = 1 ,color ='tab:blue')
ax_dict["teta"].plot(t_sim,sim_p[0]['_x','teta'],alpha = 1 ,color ='tab:blue')
# ax_dict["teta"].plot(t_sim,teta_NN_max,label = "teta_NN",color='tab:orange')
# ax_dict["teta"].plot(t_sim,teta_NN_min,color='tab:orange')
ax_dict["teta"].plot(t_sim,x_max_d[:,2],label = "teta_relax",color='tab:orange')
ax_dict["teta"].plot(t_sim,x_min_d[:,2],color='tab:orange')
...
ax_dict["teta_K"].plot(t_sim,dd['CL'][0]['_x','teta_K'],label = "teta_K_CL",alpha = 1 ,color ='tab:blue')
# ax_dict["teta_K"].plot(t_sim,teta_K_NN,color='black',label = "teta_K_NN")
...
ax_dict["teta_K"].plot(t_sim,sim_p[9]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[8]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[7]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[6]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[5]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[4]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[3]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[2]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[1]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
ax_dict["teta_K"].plot(t_sim,sim_p[0]['_x','teta_K'],alpha = 1 ,color = 'tab:blue')
# ax_dict["teta_K"].plot(t_sim,teta_K_NN_max,label = "teta_K_NN",color='tab:orange')
# ax_dict["teta_K"].plot(t_sim,teta_K_NN_min,color='tab:orange')
ax_dict["teta_K"].plot(t_sim,x_max_d[:,3],label = "teta_K_relax",color='tab:orange')
ax_dict["teta_K"].plot(t_sim,x_min_d[:,3],color='tab:orange')
...
plt.savefig("nominal_comp_dev_relaxed.pdf", format="pdf")
plt.savefig("nominal_comp_dev_relaxed.svg", format="svg")
